...
program define rdpermute, eclass 
	...
	*review optional inputs
	...
	if (!inlist("`reg'" ,"cct", "regress")){
		di as error "Error: Regression Type has to be <cct> or <regress>"
		exit
	}
qui {
...
qui {	
	...
	*Calculation of values for each kink
	foreach kink of numlist `placebo_disconts' {
		*Progress bar update
		...
		g rdpermute_kink_x_1 = rdpermute_x_1 * (rdpermute_x_1 >= 0) 
		g rdpermute_kink_x_2 = rdpermute_x_1^2 * (rdpermute_x_1 >= 0)
			...
			*Fan and Gijbels -- used by Card, Lee, Pei and Weber
			if `deriv_discont' == 1 {
				if ("`bw'" == "fg_aic") {
					...
					*Add succsecivly one variable more to the model
					forvalues p = 2/20 {
		...
		*Bandwidth choice cct:
		if ("`bw'" == "cct") {
			local bw_kernel = "uniform"
			local bw_bwselect = "mserd"
			local bw_vce = "hc0"
			if(`deriv_discont' == 0){
				local bw_kernel= "triangular"
			}
			local bw_c = 0
			local bw_scaleregul = 1
			*analyze alternative parameters for rdbwselect
			if ("`cct_bw_par'" !=  ""){
				foreach par1name in "bw_c" "bw_fuzzy" "bw_covs" "bw_kernel" "bw_weights" "bw_bwselect" "bw_vce" "bw_scaleregul" {
					local from = strpos("`cct_bw_par'", "<"+"`par1name'"+">")
					local to = strpos("`cct_bw_par'", "</"+"`par1name'"+">") - `from' 
					if(`to' != 0){ 
						local `par1name' = substr("`cct_bw_par'", `from' + strlen("`par1name'") + 2 , `to' - strlen("`par1name'") - 2 )
					}
				}
			}
			if ("`linear'"~="") {
				qui rdbwselect rdpermute_y rdpermute_x_1, kernel(`bw_kernel') deriv(`deriv_discont')  bwselect(`bw_bwselect') p(1) q(2) vce(`bw_vce') c(`bw_c') fuzzy(`bw_fuzzy') covs(`bw_covs') weights(`bw_weights') scaleregul(`bw_scaleregul')
				local h_linear = e(h_`bw_bwselect')
							}
			if ("`quad'"~="") {
				qui rdbwselect rdpermute_y rdpermute_x_1, kernel(`bw_kernel') deriv(`deriv_discont')  bwselect(`bw_bwselect') p(1) q(2) vce(`bw_vce') c(`bw_c') fuzzy(`bw_fuzzy') covs(`bw_covs') weights(`bw_weights') scaleregul(`bw_scaleregul')
				local h_quad = e(h_`bw_bwselect')
			}
			if ("`cubic'"~="") {
				qui rdbwselect rdpermute_y rdpermute_x_1, kernel(`bw_kernel') deriv(`deriv_discont')  bwselect(`bw_bwselect') p(1) q(2) vce(`bw_vce') c(`bw_c') fuzzy(`bw_fuzzy') covs(`bw_covs') weights(`bw_weights') scaleregul(`bw_scaleregul')
				local h_cubic = e(h_`bw_bwselect')
			}
		}
		...
		******************
		...
		if ("`reg'" == "regress" ) {
			...
			foreach p in `linear' `quad' `cubic'{ //Regression for 'cubic is missing, althought it is specified in the bandwith selection 
				...
				local reg_kernel = "uniform"
				if(`deriv_discont' == 0){
					reg_kernel= "triangular"
				}
				...
				*analyize alternative parameters for rdrobust
				...
				rdrobust rdpermute_y rdpermute_x_1,  h(`h_`p'') p(`p_n') q(`q_n') deriv(`deriv_discont') kernel("`reg_kernel'")  bwselect("`reg_bwselect'") b("`reg_b'") vce("`reg_vce'") fuzzy("`reg_fuzzy'") covs("`reg_covs'") weights("`reg_weights'") 
	...
	foreach p in `linear' `quad' `cubic' {
		...
		`noi' di as result "pvalues for local `p' model"
			...
			*randomization-based SEs
			...
			`noi' di as result "p-value random: " 2*min(rank_tmp_pos[true_discont_index]/`kinks_n',1-rank_tmp_pos[true_discont_index]/`kinks_n')
	...
	******************
	...
	if "`filename'"!=""{	
		...
		qui `noi' save `save_path'`filename'`dgp', replace
	}
